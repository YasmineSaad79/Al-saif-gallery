...
class StatsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cards = [
      ...
      _StatCardData(prefix: 'SAR ', target: 758.8, suffix: 'M',  title: l.statsRevenue,      subtitle: l.statsRevenueSubtitle,   isDecimal: true),
            ...
            return Row(
              children: List.generate(orderedCards.length, (i) => Expanded(
                child: Padding(
                  ...
                  child: _AnimatedStatCard(data: orderedCards[i], animate: animate),
                ),
              )),
            );
          } else {
              ...
              children: [
                IntrinsicHeight(
                  child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    Expanded(child: _AnimatedStatCard(data: orderedCards[0], animate: animate)),
                    const SizedBox(width: 16),
                    Expanded(child: _AnimatedStatCard(data: orderedCards[1], animate: animate)),
                  ]),
                ),
                const SizedBox(height: 16),
                IntrinsicHeight(
                  child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    Expanded(child: _AnimatedStatCard(data: orderedCards[2], animate: animate)),
                    const SizedBox(width: 16),
                    Expanded(child: _AnimatedStatCard(data: orderedCards[3], animate: animate)),
                  ]),
                ),
              ],
            );
...
class _AnimatedStatCard extends StatefulWidget {
  ...
  final bool animate;
  const _AnimatedStatCard({required this.data, required this.animate});
        ...
        children: [
          AnimatedBuilder(
            ...
            builder: (context, _) {
              ...
              return Text(
                '${widget.data.prefix}$display${widget.data.suffix}',
          ...
          Text(widget.data.title,
            textAlign: TextAlign.center,
            style: const TextStyle(color: Color(0xFF101727), fontSize: 10, fontWeight: FontWeight.w700, height: 1.48),
          ),
          const SizedBox(height: 3),
          Text(widget.data.subtitle,
            textAlign: TextAlign.center,
            style: const TextStyle(color: Color(0xFF495565), fontSize: 8, fontWeight: FontWeight.w400, height: 1.40),
          ),
        ],
      ),
    );
